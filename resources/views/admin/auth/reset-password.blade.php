                ...
                <div class="col-12 col-sm-8 offset-sm-2 col-md-6 offset-md-3 col-lg-6 offset-lg-3 col-xl-4 offset-xl-4">
                    ...
                    <div class="card card-primary">
                        ...
                        @if(session()->hasAny('success'))
                            <span class="float-left">{{session('success')}}</span>
                        @endif
                        @if(session()->hasAny('error'))
                            <span class="float-left">{{session('error')}}</span>
                        @endif
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" action="{{route('admin.reset-password.send')}}" class="needs-validation" novalidate="">
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="email" type="email" class="form-control" name="email" tabindex="1" required autofocus readonly value="{{@request()->email}}">
                                    <input id="email" type="hidden" class="form-control" name="token" tabindex="1" required autofocus
                                    value="{{$token}}">
                                    @error('email')
                                    <code>{{$message}}</code>
