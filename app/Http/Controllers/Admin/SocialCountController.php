...
class SocialCountController extends Controller implements HasMiddleware
{
    ...
    public static function middleware(): array
    {
        return [
            ...
            new Middleware(\Spatie\Permission\Middleware\PermissionMiddleware::using('social count  index,admin'), only:['index']),
            new Middleware(\Spatie\Permission\Middleware\PermissionMiddleware::using('social count  create,admin'), only:['create','store']),
            new Middleware(\Spatie\Permission\Middleware\PermissionMiddleware::using('social count  update,admin'), only:['edit','update']),
            new Middleware(\Spatie\Permission\Middleware\PermissionMiddleware::using('social count  delete,admin'), only:['destroy']),
